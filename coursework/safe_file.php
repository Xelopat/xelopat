<?php
// unsafe_include.php – демонстрация LFI с ограничением перехода выше на один уровень

// Определяем базовую директорию с файлами книг (папка "books" должна находиться рядом с этим скриптом)
$baseDir = realpath(__DIR__ . '/books');
if ($baseDir === false) {
    die("Ошибка: директория 'books' не найдена.");
}

// Функция, запрещающая наличие последовательности '../../'
function isTraversalAllowed($input) {
    return strpos($input, '../../') === false;
}

// Получаем параметр "book" из GET-запроса. Если не указан, используем пустую строку.
$book = isset($_GET['book']) ? $_GET['book'] : '';

$targetPath = ($book !== '') ? realpath($baseDir . '/' . $book) : false;
if ($book !== '' && ($targetPath === false || strpos($targetPath, $baseDir) !== 0)) {
    die("Ошибка: доступ к файлу запрещён.");
}
?>
<!DOCTYPE html>
<html lang="ru">
...
<body>
    ...
    <p>Выберите книгу из списка или введите название файла вручную. Полная защита от инъекции LFI</p>
