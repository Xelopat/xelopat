...
crypto_page_start('Проверка подписи ГОСТ Р 34.10-2012');
?>
    ...
    <script>
        ...
        document.getElementById('verify').addEventListener('click', () => {
            ...
            let steps = [];
            try {
                const eInv = modInverse(e, n);
                steps.push(`<b>1.</b> Вычисление V = e^(-1) mod n = ${eInv}`);

                const U1 = mod(S * eInv, n);
                steps.push(`<b>2.</b> U1 = S * V mod n = ${U1}`);

                const U2 = mod((-r) * eInv, n);
                steps.push(`<b>3.</b> U2 = -r * V mod n = ${U2}`);
